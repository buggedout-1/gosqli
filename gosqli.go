...
func processHTTPRequest(ctx context.Context, cancel context.CancelFunc, httpReq *HTTPRequest, payloads []string, responseFlag, verify, verifyDelay, retries int, noColor bool, stop int, wg *sync.WaitGroup, mu *sync.Mutex, stopOnce *sync.Once, maxConcurrency int, requiredCount int, proxy string, filename string, output bool, tolerance float64) {
	defer wg.Done()
					...
					if verify > 1 {
						...
						if isDiffVerified {
							...
							default:
								...
								// Save confirmed SQLI request to files if output flag is enabled
								if output {
									...
									// Save request with * marker
									_, err = saveConfirmedRequest(modifiedReq, httpReq, filename, false)
									if err != nil {
										fmt.Printf(Yellow("Warning: Failed to save request: %v\n"), err)
									}
								}
		...
		if isDirectory(*requestFile) {
			...
			for _, filePath := range requestFiles {
				...
				go func(filePath string) {
					...
					processHTTPRequest(ctx, cancel, httpReq, payloads, *responseFlag, *verify, *verifyDelay, *retries, *noColor, *stop, &wg, &mu, stopOnce, *maxConcurrency, *requiredCount, *proxy, filename, *output, *tolerance)
				}(filePath)
			}
			wg.Wait()
			return
		...
		for scanner.Scan() {
			...
			for _, paramURL := range paramURLs {
				...
				go func(pURL string) {
					...
					processURL(ctx, cancel, pURL, payloads, *responseFlag, *verify, *verifyDelay, *retries, *noColor, *userAgent, *stop, &wg, &mu, stopOnce, *maxConcurrency, *requiredCount, *proxy, *output, *tolerance)
				}(paramURL)
			}
		}
		wg.Wait()
